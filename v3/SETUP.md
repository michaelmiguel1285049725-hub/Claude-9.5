...
| 功能 | 需要配置吗 | 说明 |
|---|---|---|
| 地图、卡片、搜索、编号 | 不需要 | 打开就能用 |
| **笔记、随手记** | **不需要** | 默认存在这台设备的浏览器里，设置面板里可一键导出成 Markdown 备份 |
| 问 AI | 需要 | 必须有 Anthropic 密钥，且密钥不能放在公开网页里，所以要下面的"传达室" |
| 深入（语料） | 不需要配置 | 但要你自己写 `corpus/<节点id>.md` |

**所以：只想先记笔记的话，下面三个阶段都可以不做。** 缺点是本机笔记不会在手机和电脑之间同步，清空浏览器数据会丢（记得偶尔导出一次）。想要跨设备同步 + 在 Notion 里整理，再做下面的配置。
